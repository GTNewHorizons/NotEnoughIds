...
public class VanillaS26PacketMapChunkBulk implements IClassNodeTransformer {
	...
	public void transform(ClassNode cn) {
		...
		method.instructions.insertBefore(
			AsmUtil.getRelative(infalte, -2),
			new VarInsnNode(ALOAD, 0)
		);
		method.instructions.insert(
			infalte,
			Name.s26_readSize.putField()
		);
		
		AbstractInsnNode afterArrayCopy = AsmUtil.findMethodInsnNode(method.instructions.getFirst(), "arraycopy", null).getNext();
