...
public class UndergroundBiomesBiomeUndergroundDecorator implements IClassNodeTransformer {
	...
	private void transformReplaceChunkOres(ClassNode cn, MethodNode method, int varOffset) {
		...
		for (ListIterator<AbstractInsnNode> iterator = code.iterator(); iterator.hasNext();) {
			...
			if (insn.getOpcode() == Opcodes.INVOKEVIRTUAL && Name.ebs_getBlockLSBArray.matches((MethodInsnNode) insn)) { //replacing: int blockID = extendedblockstorage.getBlockLSBArray()[(inLevelY << 8 | chunkz << 4 | chunkx)] & 0xFF;
				iterator.remove();
				...
				iterator.add(Name.hooks_getBlockId.staticInvocation());
