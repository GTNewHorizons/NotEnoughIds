...
public class IETransformer implements IClassTransformer {
	...
	public byte[] transform(String name, String transformedName, byte[] basicClass) {
			...
			for (MethodNode method : cn.methods) {
				...
				for (ListIterator<AbstractInsnNode> iterator = code.iterator(); iterator.hasNext();) {
					...
					if (insn.getOpcode() == Opcodes.SIPUSH && insn.getType() == insn.INT_INSN
						...
						toInsert.set(insn, new FieldInsnNode(Opcodes.GETSTATIC, "ru/fewizz/idextender/Hooks",
								"NUM_BLOCK_IDS", "I"));
		...
		if ("cpw.mods.fml.common.registry.GameData".equals(transformedName)
				|| "cpw.mods.fml.common.registry.FMLControlledNamespacedRegistry".equals(transformedName)
				|| "cpw.mods.fml.common.registry.GameRegistry".equals(transformedName)) {
			///////////////////////////////
						...
						if (insn.getType() == insn.LDC_INSN && ((LdcInsnNode) insn).cst.equals("Y")) {
							...
							while (true) {
								if (insn.getNext().getOpcode() == Opcodes.INVOKEVIRTUAL
										&& ((MethodInsnNode) insn.getNext()).name.equals(!isObf ? "setByteArray" : "a")
							...
							toInsert.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "ru/fewizz/idextender/Hooks",
									"writeChunkToNbt",
									!isObf ? "(Lnet/minecraft/world/chunk/storage/ExtendedBlockStorage;Lnet/minecraft/nbt/NBTTagCompound;)V"
											: "(Lapz;Ldh;)V",
									false));
							...
							while (true) {
								if (insn.getNext().getOpcode() == Opcodes.INVOKEVIRTUAL
										&& ((MethodInsnNode) insn.getNext()).name
												.equals(!isObf ? "setBlockMSBArray" : "a")
										&& ((MethodInsnNode) insn.getNext()).desc.equals(
												!isObf ? "(Lnet/minecraft/world/chunk/NibbleArray;)V" : "(Lapv;)V")) {// (Lnet/minecraft/world/chunk/NibbleArray;)V
									insn = insn.getNext();
									method.instructions.remove(insn.getPrevious());
									insn = insn.getNext();
									method.instructions.remove(insn.getPrevious());
				...
				if ("<init>".equals(method.name) || "ExtendedBlockStorage".equals(method.name)) {
					...
					for (ListIterator<AbstractInsnNode> iterator = code.iterator(); iterator.hasNext();) {
						...
						toInsert.add(new FieldInsnNode(Opcodes.PUTFIELD,
								!isObf ? "net/minecraft/world/chunk/storage/ExtendedBlockStorage" : "apz",
								"block16BArray", "[S"));
					...
					for (ListIterator<AbstractInsnNode> iterator = code.iterator(); iterator.hasNext();) {
						...
						toInsert.add(
								new MethodInsnNode(Opcodes.INVOKESTATIC, "ru/fewizz/idextender/Hooks", "getBlockById",
										!isObf ? "(IIILnet/minecraft/world/chunk/storage/ExtendedBlockStorage;)Lnet/minecraft/block/Block;"
												: "(IIILapz;)Laji;",
										false));
					...
					for (ListIterator<AbstractInsnNode> iterator = code.iterator(); iterator.hasNext();) {
						...
						if (insn.getOpcode() == Opcodes.F_APPEND && insn.getNext().getOpcode() == Opcodes.ILOAD
							...
							toInsert.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL,
									!isObf ? "net/minecraft/world/chunk/storage/ExtendedBlockStorage" : "apz",
									!isObf ? "getBlockByExtId" : "a",
									!isObf ? "(III)Lnet/minecraft/block/Block;" : "(III)Laji;", false));
							method.instructions.insert(insn, toInsert);
						}

						if (insn.getOpcode() == Opcodes.INVOKESTATIC
								&& ((MethodInsnNode) insn).name.equals(!isObf ? "getIdFromBlock" : "b")
								&& insn.getNext().getOpcode() == Opcodes.ISTORE
								&& ((VarInsnNode) insn.getNext()).var == 7 && ((MethodInsnNode) insn).desc
										.equals(!isObf ? "(Lnet/minecraft/block/Block;)I" : "(Laji;)I")) {
							InsnList toInsert = new InsnList();
							...
							toInsert.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "ru/fewizz/idextender/Hooks", "hook1",
									!isObf ? "(IIIILnet/minecraft/world/chunk/storage/ExtendedBlockStorage;)V"
											: "(IIIILapz;)V",
									false));
							method.instructions.insert(insn.getNext(), toInsert);
							break;
				...
				if ("func_149269_a".equals(method.name)
					...
					for (ListIterator<AbstractInsnNode> iterator = code.iterator(); iterator.hasNext();) {
						...
						if (insn.getOpcode() == Opcodes.INVOKEVIRTUAL
								&& ((MethodInsnNode) insn).name.equals(!isObf ? "getBlockLSBArray" : "g")) {
							InsnList toInsert = new InsnList();

							toInsert.set(insn,
									new MethodInsnNode(Opcodes.INVOKESTATIC, "ru/fewizz/idextender/Hooks",
											"get16BitBlockArray",
											!isObf ? "(Lnet/minecraft/world/chunk/storage/ExtendedBlockStorage;)[B"
													: "(Lapz;)[B",
											false));
					...
					for (ListIterator<AbstractInsnNode> iterator = code.iterator(); iterator.hasNext();) {
						...
						if (insn.getOpcode() == Opcodes.ALOAD && insn.getNext().getOpcode() == Opcodes.GETFIELD
							...
							if (i != 1) {
								...
								toInsert.add(new VarInsnNode(Opcodes.ALOAD, 0));
								toInsert.add(new FieldInsnNode(Opcodes.GETFIELD,
										!isObf ? "net/minecraft/world/chunk/Chunk" : "apx",
										!isObf ? "storageArrays" : "u",
										!isObf ? "[Lnet/minecraft/world/chunk/storage/ExtendedBlockStorage;"
												: "[Lapz;"));
								toInsert.add(new VarInsnNode(Opcodes.ILOAD, 8));
								toInsert.add(new InsnNode(Opcodes.AALOAD));
								toInsert.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "ru/fewizz/idextender/Hooks",
										"set16BitBlockArray",
										!isObf ? "([BLnet/minecraft/world/chunk/storage/ExtendedBlockStorage;)V"
												: "([BLapz;)V",
										false));
			...
			for (MethodNode method : cn.methods) {
				if ("<init>".equals(method.name) || "S22PacketMultiBlockChange".equals(method.name)
					...
					for (ListIterator<AbstractInsnNode> iterator = code.iterator(); iterator.hasNext();) {
						...
						if (insn.getOpcode() == Opcodes.INVOKEVIRTUAL
								&& ((MethodInsnNode) insn).name.equals(!isObf ? "getBlock" : "a")
								&& ((MethodInsnNode) insn).desc
										.equals(!isObf ? "(III)Lnet/minecraft/block/Block;" : "(III)Laji;")) {
							InsnList toInsert = new InsnList();
							...
							toInsert.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL,
									!isObf ? "net/minecraft/world/chunk/Chunk" : "apx", !isObf ? "getBlock" : "a",
									!isObf ? "(III)Lnet/minecraft/block/Block;" : "(III)Laji;", false));
							toInsert.add(new MethodInsnNode(Opcodes.INVOKESTATIC,
									!isObf ? "net/minecraft/block/Block" : "aji", !isObf ? "getIdFromBlock" : "b",
									!isObf ? "(Lnet/minecraft/block/Block;)I" : "(Laji;)I", false));
							...
							toInsert.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL,
									!isObf ? "net/minecraft/world/chunk/Chunk" : "apx",
									!isObf ? "getBlockMetadata" : "c", "(III)I", false));
							toInsert.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL, "java/io/DataOutputStream",
									"writeByte", "(I)V", false));
			...
			for (MethodNode method : cn.methods) {
				if ("replaceChunkOres".equals(method.name) && method.desc.equals("(IILnet/minecraft/world/World;)V")) {
					...
					for (ListIterator<AbstractInsnNode> iterator = code.iterator(); iterator.hasNext();) {
						...
						if (insn.getOpcode() == Opcodes.INVOKEVIRTUAL
								&& ((MethodInsnNode) insn).name.equals(!isObf ? "getBlockLSBArray" : "func_76658_g")) {
							InsnList toInsert = new InsnList();
